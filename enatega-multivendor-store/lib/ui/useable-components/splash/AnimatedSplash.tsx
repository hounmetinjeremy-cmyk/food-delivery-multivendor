 ...
 *
 * Shared with the customer app so the rider splash looks/behaves identically.
 */
...
export default function AnimatedSplash({
  ...
  const coverScale =
    (2.3 *
      Math.sqrt(
        Math.pow(width / 2, 2) + Math.pow(Math.max(CY, height - CY), 2),
      )) /
    wipeBase;
  ...
  useEffect(() => {
    ...
    ring2.value = withDelay(
      350,
      withTiming(1, { duration: 700, easing: OUT_CUBIC }),
    );
    pinOp.value = withDelay(
      150,
      withTiming(1, { duration: 400, easing: OUT_CUBIC }),
    );
    pinIn.value = withDelay(
      150,
      withTiming(1, { duration: 650, easing: BACK_OUT }),
    );
    glowOp.value = withDelay(
      300,
      withTiming(0.9, { duration: 650, easing: OUT_CUBIC }),
    );
    ...
    wmReveal.value = withDelay(
      850,
      withTiming(1, { duration: 650, easing: INOUT_CUBIC }),
    );
    underline.value = withDelay(
      1300,
      withTiming(1, { duration: 550, easing: OUT_CUBIC }),
    );
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      ...
      whitePin.value = withDelay(
        140,
        withTiming(1, { duration: 260, easing: INOUT_CUBIC }),
      );
      if (onFinish) {
  ...
  return (
    <Animated.View style={styles.root} pointerEvents="none">
      ...
      >
        <Animated.Image
          source={PIN}
          style={[styles.fill, pinColorStyle]}
          resizeMode="contain"
        />
        <Animated.Image
          source={PIN}
      ...
      <Animated.View
        style={[
          styles.center,
          wmWrapStyle,
          { width: 252 * sf, height: 42 * sf, top: CY + 110 * sf },
        ]}
      >
