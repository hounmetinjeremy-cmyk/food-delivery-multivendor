...
export default function LanguageMain() {
  ...
  const handleLanguageSelection = async (selectedLanguage: string) => {
    ...
    await AsyncStorage.setItem("zego-language", selectedLanguage);
  };
  const handleSetCurrentLanguage = async () => {
    try {
      const lng = await AsyncStorage.getItem("zego-language");
      if (lng) {
        setIsSelected(lng);
      }
    } catch (error) {
  ...
  const handleSubmission = async () => {
    try {
      ...
      await AsyncStorage.setItem("zego-language", isSelected);
