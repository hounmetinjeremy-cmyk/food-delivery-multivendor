...
//
// Native (ios/android) sign-in uses the native Google/Apple SDKs, which have
// no web equivalent. expo-auth-session's browser-popup flow (already used by
// the iOS variant, via expo-web-browser) is the one Google sign-in path that
// works unmodified on web, so this file reuses that flow rather than the
// native-module one from useCreateAccount.android.js. Everything else
// (mutateLogin, onCompleted/onError, navigation) is identical business logic
// to both native variants, unchanged.
