...
const SelectionOnMap = ({ currentTheme, themeContext, setState, addressDetail, loading, mapRef, initialLocation, onRegionChangeComplete, setCurrentLocation, DEFAULT_LATITUDE, DEFAULT_LONGITUDE, LATITUDE_DELTA, LONGITUDE_DELTA, insets }) => {
  return (
    <View style={{ flex: 1, position: 'relative' }}>
      <View style={[styles(currentTheme).buttonContainer, { paddingTop: insets?.top !== 0 ? insets?.top : 10 }]}>
        <AntDesign
          onPress={() => {
            setState('searching')
          }}
