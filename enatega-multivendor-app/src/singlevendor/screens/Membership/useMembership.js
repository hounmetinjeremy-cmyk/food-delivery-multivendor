...
const useMembership = () => {
  ...
  async function onError(error) {
    ...
    Alert.alert(
      t('Subscription'),
      error?.localizedMessage || error?.message || t('Something went wrong, please try again!'),
      [
        {
          text: 'Ok',
  ...
  async function onCompleted(data) {
    try {
      ...
      const message =
        data?.createSubscription?.message ||
        data?.cancelSubscription?.message ||
        data?.updateSubscription?.message
      if (message) FlashMessage({ message: t(message) })
    } finally {
      setisLoading(false)
    }
  }
